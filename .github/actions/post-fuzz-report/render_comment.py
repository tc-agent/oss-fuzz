...
def _oss_fuzz_links_line(project: str) -> str:
  """Inline links for the project's Fuzz Introspector page and build status.

  Both URLs are deterministic from the project name, so we always emit them
  when the project is known — saves the agent from re-pasting them in PR
  bodies. For projects not in OSS-Fuzz the introspector link 404s, which is
  fine: the renderer is only invoked for OSS-Fuzz-tracked work.
  """
  introspector = (
      f"https://introspector.oss-fuzz.com/project-profile?project={project}")
  build_status = (
      f"https://oss-fuzz-build-logs.storage.googleapis.com/index.html#{project}"
  )
  return (f"**OSS-Fuzz:** [Fuzz Introspector]({introspector}) · "
...
def render(
  ...
  # Project-wide OSS-Fuzz links: project name is enough to derive both URLs,
  # so the agent doesn't need to re-add them in every PR body.
  if project != "?":
    out += [_oss_fuzz_links_line(project), ""]
